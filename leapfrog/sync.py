...
DEFAULT_SYNC_GITHUB_REPO = "casperson/leapfrog-segments"
...
async def push_segments_to_library(
    ...
    github_token = (
        (
            (sync_config or {}).get("github_token")
            or os.environ.get("LEAPFROG_SYNC_GITHUB_TOKEN")
            or ""
        ).strip()
    )
    ...
    if not github_token:
        raise RuntimeError("Upload requires LEAPFROG_SYNC_GITHUB_TOKEN on the server")
